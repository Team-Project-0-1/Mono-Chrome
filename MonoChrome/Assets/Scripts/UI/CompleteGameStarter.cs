    ...
    public class CompleteGameStarter : MonoBehaviour
    {
        ...
        private IEnumerator Step2_SwitchToDungeonPanel()
        {
            ...
            try
            {
                ...
                {
                    gameManager.UIManager.OnPanelSwitched("DungeonPanel");
                    LogDebug("UIManager panel switch executed");
                }
            }
            catch (System.Exception ex)
            {
        ...
        private IEnumerator Step3_ChangeGameState()
        {
            ...
            try
            {
                ...
                if (gameManager != null)
                {
                    gameManager.ChangeState(GameManager.GameState.Dungeon);
        ...
        private IEnumerator Step4_GenerateDungeon()
        {
            ...
            try
            {
                var dungeonManager = MonoChrome.Dungeon.DungeonManager.Instance ??
                                     FindObjectOfType<MonoChrome.Dungeon.DungeonManager>();
                ...
                {
                    LogError("DungeonManager not found - creating test nodes manually");
                }
            }
            catch (System.Exception ex)
            {
        ...
        public void TestNodeClick(int nodeIndex)
        {
            ...
            try
            {
                ...
                if (gameManager != null)
                {
                    gameManager.ChangeState(GameManager.GameState.Combat);
                    LogDebug("Game state changed to Combat");
                    
                    // 전투 시작
                    if (gameManager.CombatManager != null)
                    {
                        gameManager.CombatManager.InitializeCombat();
                        LogDebug("Combat initialized");
                    }
                }
            }
            catch (System.Exception ex)
